...
#define __ILTSERVER_H__
	...
	// Main functions.
		...
		// Set the CRC string so that people joining a server will have to
		// check these files before being allowed to join.
		// The format for the file list is < "CShell.DLL;CRes.DLL;Attributes\\*.txt" >.
		// The maximum length for the CRC string is 512.
		LTRESULT	(*SetCRCString)(char *szCRCFiles);

		// Calculates the CRC for all of the files in the CRC string.  This function
		// is just to allow the server to create the value at the desired times.
		LTRESULT	(*CreateStringCRC)();

		// Calculates the CRC for the currently loaded world.  This is so the
		// game can allow when to create the CRC... just in case it's not needed
		// for some worlds (ie. single player)
		LTRESULT	(*CreateWorldCRC)();
	...
	// Helpers..
		...
		// Use this just as GetNextPoly.  It will iterate over only the
		// polies in the world model.
		LTRESULT    (*GetNextWMPoly)(HPOLY *hPoly, HOBJECT hWorldModel);
	...
	// World control.
		...
		// Find all world model polies touching the box.
		// pPolyList should be an allocated array with nPolyListSize elements.
		// *pnNumPoliesFound will be set to the number of polies put into pPolyList.
		// pPolyList and/or pnNumPoliesFound may be null.

		void (*FindPoliesTouchingBox)(const LTVector & vBoxMin, const LTVector & vBoxMax, HPOLY * pPolyList, uint32 nPolyListSize, uint32 * pnNumPoliesFound,  ObjectFilterFn FilterFn, void * pUserData);
	...
	// Inter-object link manipulation.
		...
		// These are utility functions which should only be used inside LTSmartLink.
		LTRESULT (*GetSmartLinkBody)(HOBJECT hOwner, LTSmartLink_Body ** pRet);
		LTRESULT (*ReleaseSmartLinkBody)(LTSmartLink_Body * pBody);
	...
	// Object manipulation.
		...
		// Returns LT_OK, LT_NODENOTFOU ND, or LT_INVALIDPARAMS.
		...
		// LinkModelToChildModel creates an association between a child model and other child
		// models. When the child model loader is asked to load key it will load associated_chldmdl
		// instead. The names of the associated child models will be copied out of the associated_chldmdl
		// array.
		virtual void	LinkModelToExtraChildModel( char *child_model_key, char **associated_chmdl, int size_chmld) = 0 ;

		// delete all the abc-filename to child model list associations the engine has.
		virtual void	ResetModelToChildModelLink() = 0 ;
		// private to engine (don't use)
		virtual void *				GetChildModelLinkMap() = 0;
	...
	// Model manipulation.
		...
		LTBOOL	(*GetModelFilenames2)(HOBJECT hObj, ObjectCreateStruct *pStruct);
