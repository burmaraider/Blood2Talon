...
#define __ILTCLIENT_H__
	...
	// Connection stuff.
		...
		// If there is a local connection, changes
		// the game info.
		// Returns LT_NOTCONNECTED if not connected,
		//         LT_CANTCREATESERVER if not a local connection,
		//         LT_OK  if success.
		LTRESULT (*SetGameInfo)(void *pData, uint32 dataLen);
	...
	// Renderer management.
		...
		// Returns LT_OK or LT_NOTINITIALIZED.
		LTRESULT	(*ClearScreen)(LTRect *pClearRect, uint32 flags, LTVector *vColor);
	...
	// Helpers..
		...
		// You can set a global Vertex Tint that's applied to all rendering.
		// Colors are RGB 0-1 (values are clamped for you).
		void		(*GetGlobalVertexTint)(LTVector *pScale);
		void		(*SetGlobalVertexTint)(LTVector *pScale);
		void		(*OffsetGlobalVertexTint)(LTVector *pOffset);
