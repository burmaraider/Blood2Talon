...
#define __LTBASEDEFS_H__
	...
	#define END_EXTERNC() };
	...
	#if defined(__PS2) || defined(__LINUX)
		#define _MAX_PATH 256
		#define VSPRINTF(s,n,m,e) vsprintf (s,m,e)
	#else
		#define VSPRINTF(s,n,m,e) _vsnprintf (s,n,m,e)
	#endif
		...
		{ while (*s) { *s = tolower(*s); ++s; } return s; }
	...
	struct ObjectCreateStruct
	{
		...
		void	Clear()
		{
			...
			m_bResetAnimations = LTTRUE;		// Usually we want to reset animations
		}
		...
		LTBOOL			m_bResetAnimations;					// When we swap models should we reset animations?
	};
	...
	#define SURF_NONEXISTANT		(1<<1)		// Gets removed in preprocessor.
	...
	#define SURF_ALWAYSLIGHTMAP		(1<<10)		// Override for preprocessor's -allgouraud and -lightmaptogouraud.
	...
	#define SURF_XZONLY				(1<<16)		// Only DEdit cares about this.
	#define SURF_PHYSICSFIX			(1<<17)		// A dummy poly used to protect objects from moving through
	...
	#define SURF_TIMEOFDAY			(1<<20)		// Uses time of day stuff (ambient and directional light).
	...
	#define SURF_NOWALLWALK			(1<<23)		// Don't do sphere orientation collision for this surface
	#define SURF_NOBLOCKLIGHT		(1<<24)		// Don't block light with this surface
	...
	struct DeviceInput
	{
		...
		uint16			m_ControlCode;			// object instance code for the control that changed
	...
	struct DeviceBinding
	{
		...
		char			strRealName[INPUTNAME_LEN];			// real name of this trigger (##21 instead of "Space bar")
	...
	#define PLAYSOUND_CTRL_TYPE		0x2000  // Control m_nUserSoundType field.
	#define PLAYSOUND_USER_DATA		0x4000  // Control m_UserData field.
	...
	struct PlaySoundInfo
	{
		...
		// Sound type.
		// Allows game to catagorize sounds by type.  Then different types
		// can have separate operations performed on them, like ILTSoundMgr::SetVolumeByType.
		// Ignored unless PLAYSOUND_CTRL_TYPE set.  Defaults to 0 if not set.
		uint8			m_nUserSoundType;

		// User data to be passed into IClientShell::OnPlaySound.  Defaults to 0
		// if PLAYSOUND_USER_DATA not set.
		uint32		m_UserData;
    };
	...
	struct SurfaceEffectDesc
	{
		...
		void	(*UpdateEffect)(SurfaceData *pSurfaceData, void *pData);	// pData is what you returned from InitEffect.
	...
	#define FLAG2_CYLINDERPHYSICS	(1<<4)		// This object should use the cylinder-based
												// collision physics for collisions w/ the BSP
	#define FLAG2_SPHEREPHYSICS		(1<<5)		// This object should use the sphere-based collision physics
												// for collisions w/ the BSP
	#define FLAG2_ORIENTMOVEMENT	(1<<6)		// This tells the movement code to follow the edge of the BSP
												// when moving around (aka. wall walking) - NOTE: This works
												// for the spherical collision only right now
	#define FLAG2_DYNAMICDIRLIGHT	(1<<7)		// For models - re-calculates the directional lighting
												// when the object moves
	#define FLAG2_SKYOBJECT			(1<<8)		// Don't render this object thru the normal stuff,
												// only render it when processing sky objects.
	#define FLAG2_SPRITE_TROTATE	(1<<9)		// For sprites - use the object rotation as a texture rotation
	#define FLAG2_SEMISOLID			(1<<10)		// A semi-solid object is non-solid to other semi-solid objects, but solid to all else.
