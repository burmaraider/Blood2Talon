...
END_CLASS_DEFAULT_FLAGS(BaseClass, BaseClass, NULL, NULL, CF_HIDDEN)
...
// VisContainer.
// ------------------------------------------------------------------------ //
BEGIN_CLASS(VisContainer)
END_CLASS_DEFAULT_FLAGS(VisContainer, BaseClass, NULL, NULL, CF_ALWAYSLOAD | CF_HIDDEN)

// Use CF_ALWAYSLOAD eventhough object does not exist in the *.dat file.  CF_ALWAYSLOAD
// ensures that the objects won't be saved.  They are created 
// by ServerMgr::CreateVisContainerObjects().
...
uint32 DemoSkyWorldModel::EngineMessageFn(uint32 messageID, void* pData, float fData)
{
	...
	case MID_INITIALUPDATE:
	{
		...
		DWORD dwFlags = g_pLTServer->GetObjectFlags(m_hObject);
